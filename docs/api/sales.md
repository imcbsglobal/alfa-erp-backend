...
This module contains 2 main integration points:
...
### Event Structure
Each event is sent in SSE `data: <json>` format (with newline+newline at the end). Example payload (JSON):
```json
{
  "id": "9803f30d-4218-4bab-a3af-53c533afcd80",
  "amount": "124.50",
  "created_at": "2025-12-05T10:59:16.970515Z"
}
```

The `amount` must be computed by your import logic or event sender. If you need totals to appear, ensure your code builds and emits that value.
...
- `GET /api/sales/invoices/` — List invoices with pagination and filtering
- `GET /api/sales/invoices/{id}/` — Invoice detail (including items)
- `POST /api/sales/invoices/{id}/items/` — Add items
